...
@Composable
fun PassScreenAdmin(navController: NavController, pass_id: String, passViewModel: PassViewModel = viewModel()) {
            ...
            Column {
                ...
                Box(
                    ...
                    LazyColumn(
                        ...
                        passState?.let { pass ->
                            item { PassInfo1(pass = pass) }
                        }
                    }
                }
                SettingsButtons1(pass_id, passViewModel, navController)
            }
        }
    }
...
@Composable
fun SettingsButtons1(pass_id: String, passViewModel: PassViewModel, navController: NavController) {
            ...
            confirmButton = {
                Button(
                    onClick = {
                        ...
                        showDialog = false
                        navController.navigate(Screen.PassAdminScreen.route) {
                            popUpTo(Screen.PassAdminScreen.route) { inclusive = true }
                        }
                    }
                ) {
                    Text("Удалить")
...
@Composable
fun PassInfo1(pass: Pass) {
    val formattedIssueDate = formatDate1(pass.issue_date)
    val formattedExpiryDate = formatDate1(pass.expiry_date)
    val qrCodeBitmap = generateQRCode1(pass.pass_id, 1000, 1000)
...
fun generateQRCode1(content: String, width: Int, height: Int): Bitmap {
